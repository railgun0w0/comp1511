...
void NFplaycard(int a,int n,int b[],int pri[]);
...
void choose_card_to_play(void) {
    ...
    if(Ncardround==0){
        ...
        if(x==1){
            printf("%d\n",array1[0]); //yes play anycard
        }else if(notprimecard[0]!=0){
            printf("%d\n",notprimecard[0]); // do i have no prime card?
        }else{
            printf("%d\n",array1[0]);
        }
    }else{
        ...
        if(primecheck(array2[0])==0){
            // not prime 
            NFplaycard (array2[0],Ncard,notprimecard,primecard);
        }else{
            // is prime
