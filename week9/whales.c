...
int start(struct pod *first, struct date *start);
int finish(struct pod *first, struct date *finish);
...
int compare_dates(struct date first,struct date second);
int make_order(struct date first,struct date second);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    ...
    while (first_pod != NULL) {
        if (start(first_pod,start_day) == 0 && finish(first_pod,finish_day) == 0) {
...
int start(struct pod *first, struct date *start) {
    int a = 1;
    if (first->when->year > start->year) {
        a = 0;
    } else if (first->when->year == start->year && first->when->month > start->month ) {
        a = 0;
    } else if (first->when->year == start->year && first->when->month == start->month && first->when->day >= start->day){
        a = 0;
    }
    return a;
}
...
int finish(struct pod *first, struct date *finish) {
    int b = 1;
    if (first->when->year < finish->year) {
        b = 0;
    } else if (first->when->year == finish->year && first->when->month < finish->month ) {
        b = 0;
    } else if (first->when->year == finish->year && first->when->month == finish->month && first->when->day <= finish->day){
        b = 0;
    }
    return b;
}
